...
export const fetchRecommendations = async (review, { address, lng, lat }) => {
  // Check if the demo flag is true
  const useDemo = localStorage.getItem("useDemoFire") === "true";
  // Then, override user coordinates
  if (useDemo) {
  ...
  try {
    ...
    const rawResults = data.results.flatMap(
      (queryResult) => queryResult.results || []
    );

    // Select 15 amenities based on sorting function
...
// Sorting helper function (15 total, 3 of each category)
const sortAmenities = (rawResults) => {
  // Categorize Google Map Places 'type' tags
  ...
  // Categorize raw amenities based on 'type' tags
  ...
  const selected = [];
  // Select the first three amenities from each category
  selected.push(...healthcare.slice(0, 3));
  selected.push(...food.slice(0, 3));
  selected.push(...shelter.slice(0, 3));

  // Create one category for 'remaining' amenities
  const remaining = [
    ...healthcare.slice(3),
    ...food.slice(3),
    ...shelter.slice(3),
  ...
  // Fill the rest from 'remaining' until 15
